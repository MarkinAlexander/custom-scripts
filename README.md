...
Запросы идут по стандарту RFC 8484 (wireformat), который поддерживают все DoH-серверы без исключения, поэтому работает весь список целиком. Опрос последовательный: скрипт в начале сообщает параметры проверки, показывает ход выполнения и печатает результаты построчно, латентности при этом не искажаются (ускорить можно флагом `-p N`). Требуется только bash + curl.
