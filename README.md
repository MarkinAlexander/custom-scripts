...
Запросы идут по стандарту RFC 8484 (wireformat), который поддерживают все DoH-серверы без исключения, поэтому работает весь список целиком. Серверы опрашиваются параллельно небольшими партиями (по числу ядер CPU), чтобы не забивать слабый роутер и не искажать замеры. Требуется только bash + curl.
...
| `-p`, `--parallel N` | Сколько серверов опрашивать одновременно (по умолчанию — число ядер CPU, максимум 4; `1` — последовательный опрос с самыми честными латентностями). |
...
# последовательный опрос — самые точные латентности
bash dns-cheker.sh -p 1
