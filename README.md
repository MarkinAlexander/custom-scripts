...
В процессе работы скрипт запрашивает адреса GitHub обоих семейств — IPv4 и IPv6 — через Google DoH, тестирует доступность каждого и предлагает выбрать рабочий. Так определяется, через какую версию протокола у вас получается пробиться, и прописывается именно то, что работает. Если DoH не вернул адресы какого-то семейства, подставляется запасной список известных адресов GitHub. Если рабочий адрес всего один, он применится автоматически.

На роутерах **Keenetic** записи добавляются через CLI (`ndmc`), и если роутер отклоняет запись, скрипт показывает его ответ вместо молчаливого «готово». Учтите ограничение платформы: команда `ip host` в Keenetic принимает только IPv4, статических AAAA-записей в CLI нет. Поэтому при выборе IPv6 на Keenetic скрипт предупредит об этом, предложит выбрать IPv4-адрес и не станет трогать существующие записи DNS роутера (IPv6 пропишется только в `/etc/hosts` самого роутера).
...
Через IPv6 (если IPv4 заблокирован):
```bash
curl --resolve raw.githubusercontent.com:443:2606:50c0:8000::154 -o /tmp/gh_fix.sh https://raw.githubusercontent.com/MarkinAlexander/custom-scripts/main/gh_fix.sh && bash /tmp/gh_fix.sh
```
...
Скрипт проверяет публичные DoH (DNS-over-HTTPS) серверы: Google, Cloudflare, Quad9, AdGuard, NextDNS, Mullvad, Яндекс, Comss, ControlD, OpenDNS и xbox-dns. Замеряет холодную задержку (с установкой соединения) и тёплый отклик (повторный запрос по готовому соединению), показывает статус ответа и полученные IP, определяет подмену DNS провайдером (ответы stub-IP вроде `0.0.0.0` или адресов Ростелекома) и фильтрацию резолвером (пустые ответы).

Запросы идут по стандарту RFC 8484 (wireformat), который поддерживают все DoH-серверы без исключения, поэтому работает весь список целиком. Опрос последовательный: скрипт в начале сообщает параметры проверки, показывает ход выполнения и печатает результаты построчно, латентности при этом не искажаются (ускорить можно флагом `-p N`). При запуске без указания домена предлагается меню выбора: популярные и заблокированные в РФ домены или свой вариант. Требуется только bash + curl.
...
| Флаг | Описание |
|------|----------|
| *(без флагов)* | Проверка всех серверов; в интерактивном режиме сначала предлагается выбрать домен из списка (Enter — `ya.ru`), при неинтерактивном запуске берётся `ya.ru`. |
| `-d`, `--domain ДОМЕН` | Проверка с другим доменом (домен можно передать и позиционным аргументом). |
| `-t`, `--timeout СЕК` | Таймаут запроса (по умолчанию 5 сек). |
| `-p`, `--parallel N` | Сколько серверов опрашивать одновременно (по умолчанию `1` — последовательный опрос с показом прогресса; при `N>1` быстрее, но на слабых роутерах латентности искажаются). |
...
# ускорить опрос партиями по 4 (латентности могут исказиться)
bash dns-cheker.sh -p 4
...
> 💡 Колонки COLD и WARM: COLD — полный «холодный» запрос (резолв адреса самого DoH-сервера, TCP, TLS-рукопожатие и DNS-запрос; на роутере ~500 мс — норма, большую часть съедает TLS-криптография на CPU), WARM — повторный запрос по уже установленному соединению: тот отклик, который резолвер выдаёт в реальной работе (обычно десятки миллисекунд). В режиме `-m` колонка LATENCY показывает тёплый отклик.
