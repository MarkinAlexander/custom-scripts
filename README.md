...
Скрипт параллельно проверяет публичные DoH (DNS-over-HTTPS) серверы: Google, Cloudflare, Quad9, AdGuard, NextDNS, Mullvad, Яндекс, Comss, ControlD, OpenDNS и xbox-dns. Замеряет задержку, показывает статус ответа и полученные IP, определяет подмену DNS провайдером (ответы stub-IP вроде `0.0.0.0` или адресов Ростелекома) и фильтрацию резолвером (пустые ответы).
...
> 💡 Колонка LATENCY — время полного «холодного» запроса: резолв адреса самого DoH-сервера, TCP, TLS-рукопожатие и сам DNS-запрос. На роутере ~500 мс — нормальный результат, большую часть съедает TLS-криптография на CPU. Резолвер в реальной работе держит постоянное соединение, поэтому после первого запроса ответы приходят за десятки миллисекунд. Замер «тёплой» латентности (несколько запросов по одному соединению) для простой проверки избыточен — LATENCY здесь для сравнения серверов между собой.
